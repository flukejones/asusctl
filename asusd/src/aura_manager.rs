...
use crate::aura_anime::trait_impls::AnimeMatrixZbus;
...
impl DeviceManager {
            ...
            if let Some(usb_id) = usb_device.attribute_value("idProduct") {
                if let Some(vendor_id) = usb_device.attribute_value("idVendor") {
                    ...
                    if let Ok(hidraw) = HidRaw::from_device(device) {
                        ...
                        // SLASH DEVICE
                        if let Some(dev_type) = DeviceHandle::maybe_slash_hid(
                            dev.clone(),
                        ...
                        .await
                        {
                            if let DeviceHandle::Slash(slash) = dev_type.clone() {
                                ...
                                {
                                    let config = slash.lock_config().await;
                                    slash.do_initialization(&config).await?;
                                }
                        ...
                        // ANIME MATRIX DEVICE
                        if let Some(dev_type) = DeviceHandle::maybe_anime_hid(
                            dev.clone(),
                            usb_id.to_str().unwrap_or_default(),
                        )
                        .await
                        {
                            if let DeviceHandle::AniMe(mut anime) = dev_type.clone() {
                                let path =
                                    dbus_path_for_dev(&usb_device).unwrap_or(dbus_path_for_anime());
                                {
                                    anime.do_initialization().await?;
                                }
                                let ctrl = AnimeMatrixZbus::new(anime);
                                ctrl.start_tasks(connection, path.clone()).await.unwrap();
                                devices.push(AsusDevice {
                                    device: dev_type,
                                    dbus_path: path,
                        ...
                        // AURA LAPTOP DEVICE
                        if let Some(dev_type) = DeviceHandle::maybe_laptop_aura(
                            dev,
                            usb_id.to_str().unwrap_or_default(),
                        )
                        .await
                        {
                            if let DeviceHandle::Aura(aura) = dev_type.clone() {
                                ...
                                {
                                    let config = aura.lock_config().await;
                                    aura.do_initialization(&config).await?;
                                }
    ...
    async fn init_all_hid(connection: &Connection) -> Result<Vec<AsusDevice>, RogError> {
        ...
        {
            devices.append(&mut Self::maybe_hid_devices(connection, device).await?);
        }
    ...
    pub async fn find_all_devices(connection: &Connection) -> Vec<AsusDevice> {
        ...
        if do_slash {
            if let Ok(dev_type) = DeviceHandle::maybe_slash_usb() {
                ...
                if let DeviceHandle::Slash(slash) = dev_type.clone() {
                    let path = dbus_path_for_slash();
                    {
                        let config = slash.lock_config().await;
                        slash
                            .do_initialization(&config)
                            .await
                            .map_err(|e| error!("slash error: {e:?}"))
                            .ok();
                    }
                    let ctrl = SlashZbus::new(slash);
                    ctrl.start_tasks(connection, path.clone()).await.unwrap();
        ...
        if do_anime {
            if let Ok(dev_type) = DeviceHandle::maybe_anime_usb() {
                // TODO: this is copy/pasted
                if let DeviceHandle::AniMe(mut anime) = dev_type.clone() {
                ...
                for event in monitor.iter() {
                        ...
                        event.parent_with_subsystem_devtype("usb", "usb_device")?
                    {
                        ...
                        if action == "remove" {
                            ...
                            let conn_copy = conn_copy.clone();
                            block_on(async move {
                                if let Ok(mut new_devs) = Self::maybe_hid_devices(&conn_copy, evdev)
                                    .await
